...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function updateFrame() {
      ...
      updateColors(dt);
      applyInputs();
      const isIdle = now - lastInputTime > IDLE_TIMEOUT_MS;
      if (!isIdle) {
        step(dt);
        render(null);
      }
    ...
    function generateColor() {
      ...
      if (config.BRAND_COLORS && config.BRAND_COLORS.length) {
        const pick = config.BRAND_COLORS[Math.floor(Math.random() * config.BRAND_COLORS.length)];
    ...
    function handleTouchMove(e) {
      const now = performance.now();
      if (now - lastTouchMoveTime < MOVE_THROTTLE_MS) return;
